...
export class Service {
  ...
  viewStudent(classId:string){
    return this.http.get<StudentsResponse>(`http://localhost:4000/api/v1/get-My-class-students/${classId}`,{
      withCredentials:true
    })
  }
  ...
  getsingleidnotes(studentId:string){
     return this.http.get<NotesResponsesingle>(`http://localhost:4000/api/v1/getNotesByUser/${studentId}`,{
      withCredentials:true
    })
  }
editNotes(id: string, formData: FormData) {
 ...
 editstatus(data:any,id:string){
  ...
  logout(){
    return this.http.post("http://localhost:4000/api/v1/logout",{},{
      withCredentials:true
    })
  }
...
getUserByClass(userId: string, classId: string): Observable<NotesResponse> {
  return this.http.get<NotesResponse>(
    `${this.apiUrl}/user/${userId}/class/${classId}`
  );
}
